  ...
  const handleSubmit = (e) => {
    ...
    if (validator.allValid()) {
      if (mobileErr == '' || mobileErr == null) {
        setDisabled(true);
        Axios.post( API_BASE_URL + 'api/update_user_profile', formData).then((response) => {          
      ...
      .then((response) => {
        if (response.data.status == 'false') {
          ...
          setEditProfile(prevState => ({
            ...prevState,
            state: "",  // Replace newValue with the new value for the city property
          }));
        }
      })
      .catch((err) => console.log(err));
                ...
                <div className="form_area">
                  <form action="" onSubmit={handleSubmit}>
                            ...
                            >
                              <MenuItem value="Country Code" disabled>Select country code</MenuItem>
                              {
                                  countryCodeLists.map((item) => {
                                      if(item.id === '229') {
                                          return <MenuItem key={item.id} value={item.id} selected={`${editProfile.countryCode == item.id ? 'selected' : ''}`}>{item.country_name} - {item.country_code}</MenuItem>
                                      }
                                  })
                                }
                                {
                    ...
                    <div className="form_groupDiv edit_profile">
                      ...
                      />
                      <div style={errorMsg}>
                        {validator.message('city_autofill', editProfile.city_autofill, 'required', {
                          messages: {
